...
| 역할   | 이름       |작업                                                        |
|--------|------------|-----------------------------------------------------------|
| 팀장   | 김연우     | 프로젝트 초기 설계, 디렉토리 구조 및 실행 스크립트(run.sh) 구성, GPT-4o-mini 챗봇 업그레이드, 릴리스 관리|
| 팀원   | 나덕윤     | 음성 감정 모델 개발 (CNN+LSTM), MFCC 추출 및 데이터 전처리, 마이크 테스트 및 성능 개선|
| 팀원   | 심채훈     | 제스처 인식 모듈화, MLP/SVC 얼굴 감정 모델 개발, PyQt 기반 GUI 통합 |
| 팀원   | 임동욱     | 제스처 모델 학습 |
